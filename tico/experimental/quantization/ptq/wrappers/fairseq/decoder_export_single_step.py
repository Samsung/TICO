...
but the stock path is "stateful" and "shape-polymorphic": it owns and
...
class DecoderExportSingleStep(nn.Module):
    ...
    Inputs (example shapes; B=1, H=8, Dh=64, C=512, S=64, Tprev=63):
      ...
      - self_attn_mask:       [B, 1, S_tot]      (optional) additive float mask for decoder self-attn at this step; pass zeros if unused
    ...
    def forward(
        ...
        *kv_args: torch.Tensor,  # prev_k_0..L-1, prev_v_0..L-1 (total 2L tensors)
        self_attn_mask: torch.Tensor,  # [B,1,S_tot] (or zeros)
    ):
        L = self.num_layers
        H = self.num_heads
        Dh = self.head_dim
        ...
        # Call decoder's external single-step path (features only)
        # Returns:
        #   x_step: [B,1,C]
...
def make_example_inputs(*, L=4, B=1, S=64, H=8, Dh=64, C=512, Tprev=63, device="cuda"):
    """
...
# ----- 4) (Optional) Quick shape sanity after export -----------------------
def quick_run(wrapper: nn.Module):
    """
    Run the wrapper forward once with example inputs and print output shapes.
    """
    L = wrapper.num_layers
    ...
    Dh = wrapper.head_dim
    C = wrapper.embed_dim
    args = make_example_inputs(L=L, H=H, Dh=Dh, C=C)
    outs = wrapper(*args)

    # outs: (x_step, new_k0, new_v0, ..., new_k{L-1}, new_v{L-1})
    x_step = outs[0]
    kvs = outs[1:]
    print("x_step:", tuple(x_step.shape))  # (1,1,512)
    for i in range(L):
        nk = kvs[2 * i + 0]
        nv = kvs[2 * i + 1]
        print(f"layer {i} new_k:", tuple(nk.shape), "new_v:", tuple(nv.shape))
        # Expect each to be (H, B, Dh) == (8, 1, 64)
