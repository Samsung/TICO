...
def fuse_ln_linear(
    ...
    for linear in linear_layers:
        ...
        linear.weight.data = linear.weight.data * layernorm.weight  # TODO: Check
...
def rotate_model(model: torch.nn.Module, mode: str = "hadamard") -> None:
    ...
    # Rotate head
    head = model.lm_head
    head.weight.data = torch.matmul(head.weight.data, r1)
    ...
    for layer in tqdm(model.model.layers, unit="layer", desc="Rotating"):
        ...
        # Rotate `o_proj` and `v_proj` with `r2` matrix
