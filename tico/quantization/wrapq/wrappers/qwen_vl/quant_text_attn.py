...
class QuantQwen3VLTextAttention(QuantModuleBase):
    def __init__(
        ...
        *,
        ...
        self.k_norm = PTQWrapper(
            fp_attn.k_norm, qcfg=kn_cfg, fp_name=f"{fp_name}.k_norm"
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        k_rep = k_rot.repeat_interleave(self.kv_rep, dim=1)  # (B, n_h, K, H)
        v_rep = v.repeat_interleave(self.kv_rep, dim=1)  # (B, n_h, K, H)
        ...
        # Softmax
        attn = torch.softmax(logits, dim=-1, dtype=torch.float32).to(q_rot.dtype)
        attn = self._fq(attn, self.obs_softmax)

        # Attention output
        attn_out = (
            self._fq(attn @ v_rep, self.obs_attn_out).transpose(1, 2).reshape(B, S, -1)
        )  # (B, S, n_h * H)
