...
class PrefillDecodeUtils:
    ...
    def build_attention_mask_for_padded_input(self, cur_seq_len, right_padding = False, mask_value:float = -120.0, prefill: bool = True):
        dtype = torch.float32
    ...
    def build_position_embeddings_for_padded_input(self, position_embeddings, cur_seq_len, right_padding = False, prefill: bool = True):
        dtype = torch.float32
...
#
           ...
           #     dtype=dtype,
           # )
            attention_mask = self.build_attention_mask_for_padded_input(cur_seq_len, right_padding=True, prefill=False)
            ...
            attention_mask = self.build_attention_mask_for_padded_input(cur_seq_len, right_padding=False, prefill=False)
            attention_mask = attention_mask[..., -1, :].unsqueeze(0) # last row
            position_embeddings = self.build_position_embeddings_for_padded_input(self.pos_embeds, cur_seq_len, right_padding=False, prefill=False )
            (cos, sin) = position_embeddings
            cos = cos[..., -1, :].unsqueeze(0)
            sin = sin[..., -1, :].unsqueeze(0)
            position_embeddings = (cos, sin)
...
def get_decode_input(
    ...
    prefill_seq_len = calib_input.shape[-1]
    ...
    attn_mask = helper.build_attention_mask_for_padded_input(prefill_seq_len, right_padding=not left_pad)
    position_embeddings = helper.build_position_embeddings_for_padded_input(ropes, prefill_seq_len, right_padding=not left_pad)
    ...
    dec_inputs = helper.get_input_for_decode_model(next_token, past_key_values=outputs.past_key_values, cur_seq_len = prefill_seq_len + 1, right_padding=not left_pad)
    return dec_inputs
    #dtype=torch.float32
    #attention_mask = _build_decode_attention_mask(
    #    batch_size=1,
    ...
    with torch.no_grad():
        for batch in tqdm.tqdm(dataset):
            if isinstance(batch, torch.Tensor):
                ...
                #ref_output = model(prefill_input.to(device))
...
def main():
    ...
    # original_prefill_decode_ppl = evaluate_ppl_of_prefill_decode_model_on_dataset(model, model, calib_inputs, pad_token_id=tokenizer.pad_token_id, max_seq_len = args.max_seq_len, seed = args.seed, device = device)
    # print("\n┌── Wikitext-2 prefill_decode original calibration perplexity ─────────────")
    # print(f"│ fp32 : {original_prefill_decode_ppl:8.2f}")
    # print("└───────────────────────────────────────────")
