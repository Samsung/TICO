...
4. `mse_for_gptq` - `mse` adjusted for GPTQ. GPTQ modifies the matrix during the quantization process, so the most accurate method would consist in finding a quantizer that yields the smallest quantization error after the GPTQ method has been applied $MSE\_FOR\_GPTQ_{MIN, MAX}(W) = argmin_{min, max}||W-Q_{min, max}(W_{GPTQ})||^2$. Since this would be quite computationally expensive, we can use an accelerated approximate GPTQ method — FPI_GPTQ $MSE\_FOR\_GPTQ_{MIN, MAX}(W) = argmin_{min, max}||W-Q_{min, max}(W_{FPI\_GPTQ})||^2$. This is slower than `mse`/`smse` but can provide better accuracy.
