...
def resolve_gemma4_components(
    ...
    vision_encoder_layers = get_module_by_path(model, config.vision_encoder_layers_attr)
    vision_pooler = get_module_by_path(model, config.vision_pooler_attr)
    multimodal_embedder = get_module_by_path(model, config.multimodal_embedder_attr)
    ...
    if not isinstance(vision_pooler, nn.Module):
        raise TypeError(
            f"vision_pooler must be nn.Module. " f"Got {type(vision_pooler).__name__}."
        )
