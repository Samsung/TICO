...
def perplexity(model) -> float:
    ...
    with torch.no_grad():
        for i in tqdm.trange(0, ids.size(1) - 1, STRIDE):
            end = min(i + STRIDE, ids.size(1) - 1)  # leave room for next token
            L = end - i  # actual length

            inp = ids[:, i:end]  # (1,L)
            tgt = ids[:, i + 1 : end + 1]  # (1,L) same length

            logits = model(inp).logits[:, :L, :]  # keep first L positions
    ...
    return math.exp(nll_total / tok_total)


# ---------------- 5. Run and report -------------------------------
print("\nCalculating perplexities …")
if RUN_FP:
    ppl_fp = perplexity(fp_model)
else:
    ppl_int8 = perplexity(int8_model)
