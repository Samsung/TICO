...
"""Analyze W8 parameter sensitivity under the P2 W8/A16 precision ceiling."""
...
def add_subparser(
    ...
    *,
    command: str = "weight-precision-sensitivity",
) -> argparse.ArgumentParser:
    parser = subparsers.add_parser(
        command,
        help=(
            "Rank leave-one-weight-group-float P2 candidates and optionally "
            "run a classifier-constrained greedy cumulative path."
        ),
    )
    _add_arguments(parser)
    return parser
...
def _add_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        default="semantic",
        help=(
            "Analyze all Conv/DW/PReLU parameters in one semantic block or "
            "each parameter observer independently."
        ),
    )
    parser.add_argument(
        "--groups",
        nargs="+",
        help="Optional ordered subset of semantic or site group names.",
    )
    parser.add_argument(
        "--skip-greedy",
        action="store_true",
        help="Run only the independent leave-one-group-float ranking.",
    )
    parser.add_argument(
        "--greedy-include-all-groups",
        action="store_true",
        help=(
            "Let greedy re-evaluate every selected group, including groups that "
            "are independently negative or classifier-ineligible. Use with a "
            "zero candidate limit for exhaustive interaction search."
        ),
    )
    parser.add_argument(
        ...
        default=12,
        help=(
            "Use the top independently REG-positive, classifier-eligible groups "
            "for greedy search; zero keeps every eligible group."
        ),
    )
    parser.add_argument(
        "--max-greedy-steps",
        type=int,
        default=12,
        help="Maximum greedy selections; zero allows every candidate group.",
    )
    parser.add_argument(
        "--minimum-improvement",
        type=float,
        default=0.0,
        help="Minimum REG MAE improvement required for the next greedy step.",
    )
    parser.add_argument(
        "--auxiliary-tolerance",
        type=float,
        default=0.0,
        help=(
            "Maximum classifier MAE regression accepted independently and at "
            "each greedy step."
        ),
    )
    ...
    parser.add_argument(
        "--report-json",
        type=Path,
        default=DIRECTORY / "reports" / "weight_precision_sensitivity.json",
    )
...
def run(args: argparse.Namespace) -> None:
    ...
    report = run_weight_precision_sensitivity(
        float_model,
        calibration,
        evaluation,
        ...
        greedy_include_all_groups=args.greedy_include_all_groups,
        greedy_candidate_count=args.greedy_candidate_count,
    ...
    print_weight_precision_sensitivity(report, top_k=args.top_k)
...
def _validate_args(args: argparse.Namespace) -> None:
    if args.greedy_candidate_count < 0 or args.max_greedy_steps < 0:
    ...
    if args.target_regressor_mae <= 0.0:
        raise ValueError("--target-regressor-mae must be positive.")
    if args.top_k < 0:
